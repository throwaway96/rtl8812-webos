	...
	, struct cfg80211_scan_request *request)
{
	...
	if (!is_wildcard_ssid) {
		...
		if (ssid_exist) {
			for (i = 0; i < request->n_ssids && i < RTW_SSID_SCAN_AMOUNT; i++) {
				_rtw_memcpy(padapter->hidden_ssid[i].Ssid
					, ssids[i].ssid, ssids[i].ssid_len);
				padapter->hidden_ssid[i].SsidLength = ssids[i].ssid_len;
			}
		} else {
			for (i = 0; i < request->n_ssids && i < RTW_SSID_SCAN_AMOUNT; i++) {
				_rtw_memcpy(&parm.ssid[i].Ssid, padapter->hidden_ssid[i].Ssid
					, padapter->hidden_ssid[i].SsidLength);
				parm.ssid[i].SsidLength = padapter->hidden_ssid[i].SsidLength;
			}
		}
	}
#endif /* LGE_PRIVATE */
...
static int rtw_add_beacon(_adapter *adapter, const u8 *head, size_t head_len, const u8 *tail, size_t tail_len)
{
	...
	/* In pure wpa_supplicant, do not remove P2P and WFD IE */

	/* In android case, wpa_suppliacnt will call driver private
	   function wpa_driver_set_ap_wps_p2p_ie() to append new P2P and WFD IE */
		...
		struct cfg80211_beacon_data *info)
{
	...
	if (info->proberesp_ies_len > 0)
		rtw_cfg80211_set_proberesp_ies(ndev, info->proberesp_ies,
									   info->proberesp_ies_len);
#endif /* not_yet */

	if (info->assocresp_ies_len > 0)
		rtw_cfg80211_set_assocresp_ies(ndev, info->assocresp_ies,
	...
	u16 frame_type, bool reg)
{
	...
	case IEEE80211_STYPE_PROBE_REQ: /* 0x0040 */
		if (reg > 0) SET_CFG80211_REPORT_MGMT(pwdev_priv, IEEE80211_STYPE_PROBE_REQ, reg);
		else CLR_CFG80211_REPORT_MGMT(pwdev_priv, IEEE80211_STYPE_PROBE_REQ, reg);
		break;
#ifdef not_yet
	case IEEE80211_STYPE_ACTION: /* 0x00D0 */
		if (reg > 0) SET_CFG80211_REPORT_MGMT(pwdev_priv, IEEE80211_STYPE_ACTION, reg);
		else CLR_CFG80211_REPORT_MGMT(pwdev_priv, IEEE80211_STYPE_ACTION, reg);
		break;
#endif /* not_yet */
	default:
		break;
