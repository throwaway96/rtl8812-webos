...
{
	if (enable)
		hw_bcn_ctrl_set(adapter, get_hw_port(adapter), (BIT_P0_EN_RXBCN_RPT | BIT_DIS_TSF_UDT | BIT_EN_BCN_FUNCTION));
	else
		hw_bcn_ctrl_clr(adapter, get_hw_port(adapter), BIT_EN_BCN_FUNCTION_8822B);
}
...
#endif /* CONFIG_CONCURRENT_MODE */
		default:
			RTW_ERR(FUNC_ADPT_FMT" Unknow hw port(%d) \n", FUNC_ADPT_ARG(adapter),  get_hw_port(adapter));
			rtw_warn_on(1);
			break;
		}
...
void hw_var_set_dl_rsvd_page(PADAPTER adapter, u8 mstatus)
{
	...
	if (mstatus == RT_MEDIA_CONNECT) {
		...
		} while (!bcn_valid && DLBcnCount <= 2 && !RTW_CANNOT_RUN(adapter));
...
void get_tsf_by_port(_adapter *adapter, u8 *tsftr, u8 hw_port)
{
	...
	val32 |=  BIT_BCN_TIMER_SEL_FWRD(hw_port);
	...
	case HW_VAR_TSF:
		{
			...
			get_tsf_by_port(adapter, val, hw_port);
		}
		break;
