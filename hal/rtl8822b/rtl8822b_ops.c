...
u8 rtl8822b_read_efuse(PADAPTER adapter)
{
	...
	if (hal->bautoload_fail_flag) {
		RTW_INFO(CLR_LT_RED "%s: <WARN> no valid efuse data" CLR_NONE "\n", __func__);
		return _FAIL;
	}
#endif
...
static void xmit_status_check(PADAPTER p)
{
	...
	if (txdma_status != 0x00) {
		...
		hal->self_detection.tx_dma_status_cnt++;
		hal->self_detection.self_dect_case = 6;
			...
			if (psrtpriv->last_tx_complete_time == 0)
				...
				if (diff_time > 4000) {
					...
					if (!(ability & ODM_BB_ADAPTIVITY)) {
						hal->self_detection.self_dect_tx_cnt ++;
						hal->self_detection.self_dect_case = 1;
						rtw_hal_sreset_reset(p);
					}
				}
			}
...
static void check_rx_count(PADAPTER p)
{
	...
	cur_bb_cck_cnt = rtw_read16(p, 0xf04);
	cur_bb_ofdm_cnt = rtw_read16(p, 0xf14);
	cur_bb_ht_cnt = rtw_read16(p, 0xf10);
	cur_bb_vht_cnt = rtw_read16(p, 0xf0c);

		
	rtw_write8(p, REG_RXERR_RPT_8822B + 3, 0);
	cur_mac_cck_cnt = rtw_read16(p, REG_RXERR_RPT_8822B);

	rtw_write8(p, REG_RXERR_RPT_8822B + 3, 0x30);
	cur_mac_ofdm_cnt = rtw_read16(p, REG_RXERR_RPT_8822B);

	rtw_write8(p, REG_RXERR_RPT_8822B + 3, 0x60);
	cur_mac_ht_cnt = rtw_read16(p, REG_RXERR_RPT_8822B);

	rtw_write8(p, REG_RXERR_RPT_8822B + 3, 0x04);
	cur_mac_vht_cnt = rtw_read16(p, REG_RXERR_RPT_8822B);

	if (self_detection->last_bb_cck_cnt == cur_bb_cck_cnt &&
...
exit:
	...
	self_detection->last_bb_ht_cnt = cur_bb_ht_cnt;
	self_detection->last_bb_vht_cnt = cur_bb_vht_cnt;
	self_detection->last_mac_cck_cnt = cur_mac_cck_cnt;
	self_detection->last_mac_ofdm_cnt = cur_mac_ofdm_cnt;
	self_detection->last_mac_ht_cnt = cur_mac_ht_cnt;
	self_detection->last_mac_vht_cnt = cur_mac_vht_cnt;

	if (self_detection->rx_cnt > 3) {
		hal->self_detection.self_dect_case = 2;
		hal->self_detection.self_dect_rx_cnt ++;
		rtw_hal_sreset_reset(p);
	}
}
...
static void linked_status_check(PADAPTER p)
{
	...
	if (rx_dma_status != 0x00) {
		...
		hal->self_detection.rx_dma_status_cnt ++;
		hal->self_detection.self_dect_case = 7;
		rtw_hal_sreset_reset(p);
	}

	if (hal->self_detection.self_dect_fw) {
		hal->self_detection.self_dect_case = 3;
		rtw_hal_sreset_reset(p);
	}

	if ( hal->self_detection.is_txbuf_empty) {
		hal->self_detection.self_dect_case = 5;
		rtw_hal_sreset_reset(p);
	}
...
#ifdef DBG_CONFIG_ERROR_DETECT
			hal->self_detection.is_txbuf_empty = _TRUE;
			hal->self_detection.txbuf_empty_cnt++;
#endif /* DBG_CONFIG_ERROR_DETECT */
...
*/
	case HW_VAR_CH_SW_IQK_INFO_BACKUP:
		/* hw_var_backup_IQK_val(adapter, (struct hal_iqk_reg_backup *)val); */
		break;
...
#ifdef CONFIG_MCC_MODE
			/* hw_var_restore_IQK_val_MCC(adapter); */
#endif /* CONFIG_MCC_MODE */
			break;
		}
		break;
...
*/
	...
	case HW_VAR_TSF_AUTO_SYNC:
		if (*val == _TRUE) {
			/* enable TSF auto sync */
			u8 val8 = rtw_read8(adapter, REG_MISC_CTRL_8822B);
