...
#ifdef LGE_PRIVATE
			/* Ixia control frame */
			if (pattrib->udp_src == 0x46B4
				&& pattrib->udp_dst == 0x1000)
...
static s32 rtw_dump_xframe(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	...
	struct pwrctrl_priv *pwrpriv = dvobj_to_pwrctl(pdvobj);
	...
	for (t = 0; t < pattrib->nr_frags; t++) {
		...
		if (!((pattrib->qsel == QSLT_BEACON) || (pattrib->qsel == QSLT_CMD)) && (pwrpriv->bInSuspend == _TRUE))
