	...
	if (p_dm_fat_table->fat_state == FAT_DECISION_STATE) {
		...
		for (j = 0; j < (pdm_sat_table->total_beam_set_num); j++) {
			...
			ODM_RT_TRACE(p_dm_odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("Beam_Set[%d]: pkt_cnt=(( %d )), avg_rssi_MA=(( %d )), avg_rssi=(( %d ))\n",
				j, pdm_sat_table->statistic_pkt_cnt[j], avg_rssi_tmp_ma, avg_rssi_tmp));
		...
		/*[Joint Decision]-------------------------------------------------------------------*/
		ODM_RT_TRACE(p_dm_odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("---> [RSSI]      Target Beam(( %d )) RSSI_max=((%d))\n", rssi_target_beam, target_beam_max_rssi));
		ODM_RT_TRACE(p_dm_odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("---> [EVM 2SS] Target Beam(( %d )) EVM2SS_max=((%d))\n", evm2ss_target_beam, target_beam_max_evm2ss));
		ODM_RT_TRACE(p_dm_odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("---> [EVM 1SS] Target Beam(( %d )) EVM1SS_max=((%d))\n", evm1ss_target_beam, target_beam_max_evm1ss));

		if (target_beam_max_rssi <= 20) {
			pdm_sat_table->rx_idle_beam_set_idx = rssi_target_beam;
			decision_type = 1;
		} else {
			if (((beam_path_evm_2ss_cnt_all<<2) > (beam_path_evm_1ss_cnt_all)) && (beam_path_evm_2ss_cnt_all != 0)) {
		...
		for (j = 0; j < (pdm_sat_table->total_beam_set_num); j++) {
			...
			per_beam_rssi_diff_tmp = target_beam_max_rssi - pdm_sat_table->beam_set_avg_rssi_pre[j];
...
)
{
	...
	if ((p_pktinfo->data_rate >= ODM_RATEMCS8 && p_pktinfo->data_rate <= ODM_RATEMCS15) || 
		(p_pktinfo->data_rate >= ODM_RATEVHTSS2MCS0 && p_pktinfo->data_rate <= ODM_RATEVHTSS2MCS9)) {
			rate_ss = 2;
		}
	...
	/*[data]*/
		...
		if (pdm_sat_table->pkt_skip_statistic_en == 0) {
			...
			ODM_RT_TRACE(p_dm_odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("RX Rate = ((0x%x)), rate_ss = ((%d)), EVM{A,B} = {%d, %d}\n", p_pktinfo->data_rate, rate_ss,  rx_evm_ant0, rx_evm_ant1));
