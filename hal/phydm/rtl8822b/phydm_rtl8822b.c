...
)
{
	...
	if ((*dm->channel == 6) && (dm->rssi_min <= rssi_h2l) && (*dm->band_width == CHANNEL_WIDTH_20)) {
		set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 20, 2440, PHYDM_DONT_CARE);
	} else {
		set_result_nbi = phydm_nbi_setting(dm, FUNC_DISABLE, *dm->channel, 20, 2440, PHYDM_DONT_CARE);
	}
	
	if ((*dm->band_width == CHANNEL_WIDTH_20) && (*dm->channel == 11) && ((dm->rfe_type == 3) || (dm->rfe_type == 17)) && (dm->rssi_a < 40)) {
		odm_set_bb_reg(dm, 0x830, BIT(13)|BIT(14)|BIT(15)|BIT(16), 0xa);
		odm_set_bb_reg(dm, 0x838, BIT(4)|BIT(5)|BIT(6)|BIT(7), 0x6);
	} else {
		odm_set_bb_reg(dm, 0x830, BIT(13)|BIT(14)|BIT(15)|BIT(16), 0x7);
	}
	...
	for (i = 0;i <= 1;i++) {
		for (j = 0;j < sta_num;j++) {
			if(mcc_dm->sta_macid[i][j] != 0xff) {
				p_entry = dm->phydm_sta_info[mcc_dm->sta_macid[i][j]];
				if (p_entry == NULL) {
...
void phydm_mcc_ctrl(void *dm_void)
{
	...
	PHYDM_DBG(dm, DBG_TMP, "MCC status: %x\n", mcc_dm->mcc_status);
	/*MCC stage no change*/
