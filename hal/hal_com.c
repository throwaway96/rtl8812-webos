...
static u8 rtw_hal_set_wowlan_ctrl_cmd(_adapter *adapter, u8 enable, u8 change_unit)
{
	...
	u8 ret = _FAIL;
	u8 dis_uphy = 0, dis_uphy_unit = 0,
		dis_uphy_time = 0, dis_uphy_take_pdn = 0, wowlan_ext = 1;
	...
	SET_H2CCMD_WOWLAN_PATTERN_MATCH_ENABLE(u1H2CWoWlanCtrlParm, 0 /* enable */);
...
download_page:
	...
	/* rtw_dump_rsvd_page(RTW_DBGDUMP, adapter, adapter_to_dvobj(adapter)->mcc_objpriv.mcc_pwr_idx_rsvd_page[0], 2); */
...
#if defined(RTW_CHANNEL_SWITCH_OFFLOAD) || defined(CONFIG_MCC_MODE_V2)
/* init drvier phydm parameter to fw */
void rtw_hal_init_fw_phydm_general_info(_adapter *adapter)
{
	rtw_halmac_send_general_info(adapter_to_dvobj(adapter));
}
