...
#ifdef LGE_PRIVATE
void rtw_dump_fw_version(void *sel, _adapter *adapter)
{
	...
	if (hal_data->bFWReady)
		snprintf(sel, 6, "%02d.%02d", hal_data->firmware_version, hal_data->firmware_sub_version);
	else
		snprintf(sel, 6, "FF.FF");
}
#endif /* LGE_PRIVATE */
...
chk_hw_country_code:
	if (hw_alpha2 && !IS_ALPHA2_NO_SPECIFIED(hw_alpha2)) {
		ent = rtw_get_chplan_from_country(hw_alpha2, 0);
		if (ent) {
...
chk_sw_config:
	...
	if (sw_alpha2 && !IS_ALPHA2_NO_SPECIFIED(sw_alpha2)) {
		ent = rtw_get_chplan_from_country(sw_alpha2, 0);
		if (ent) {
...
static u8 rtw_hal_set_wowlan_ext_cmd(_adapter *adapter, u8 enable)
{
	struct hal_ops *pHalFunc = &adapter->hal_func;
	u8 u1H2CDisconDecisionParm[H2C_WOW_EXT_LEN] = {0};
	u8 ret = _FAIL;

	SET_H2CCMD_WOWLAN_EXT_ENABLE(u1H2CDisconDecisionParm, enable);
	SET_H2CCMD_CUSTOM_HST2DEV_NUM(u1H2CDisconDecisionParm, HST2DEV_GPIO_IDX);
...
static u8 rtw_hal_set_wowlan_ctrl_cmd(_adapter *adapter, u8 enable, u8 change_unit)
{
	...
	SET_H2CCMD_RISE_HST2DEV(u1H2CWoWlanCtrlParm, 1);
#endif /* CONFIG_RTW_ONE_PIN_GPIO */
	SET_H2CCMD_WOWLAN_TIME_FOR_UPHY_DISABLE(u1H2CWoWlanCtrlParm, dis_uphy_time);
#ifndef CONFIG_RTW_ONE_PIN_GPIO
	SET_H2CCMD_WOWLAN_EXT_EN(u1H2CWoWlanCtrlParm, wowlan_ext);
#endif /* !CONFIG_RTW_ONE_PIN_GPIO */
#endif /* CONFIG_DIS_UPHY */

	SET_H2CCMD_DIS_INBAND(u1H2CWoWlanCtrlParm, 1);
...
#ifndef CONFIG_RTW_ONE_PIN_GPIO
	rtw_hal_set_wowlan_ext_cmd(padapter, enable);
#endif /* !CONFIG_RTW_ONE_PIN_GPIO */
...
download_page:
	...
	/* rtw_dump_rsvd_page(RTW_DBGDUMP, adapter, 0, 10); */
	return;
error:
	rtw_free_xmitframe(pxmitpriv, pcmdframe);
}
...
#ifdef LGE_PRIVATE
u8 rtw_get_current_txpwr(_adapter *padapter, struct sta_info *psta)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
	struct dm_struct *pDM_Odm = &pHalData->odmpriv;
	struct ra_table *pRA_Table = &pDM_Odm->dm_ra_table;
	...
	current_rate_id = rtw_get_current_tx_rate(padapter, psta);
			...
			while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
				...
				if (psta) {
					...
					if ((_rtw_memcmp(psta->cmn.mac_addr, bc_addr, 6) !=   _TRUE)
						...
						case CHANNEL_WIDTH_20:
							BW = "20 MHz";
							if (psta_dframe_info->sta_sgi)
								phy_rate = phyrate_tbl[psta_dframe_info->sta_data_rate].bw20_sgi_phyrate;
							else
						...
						case CHANNEL_WIDTH_40:
							BW = "40 MHz";
							if (psta_dframe_info->sta_sgi)
								phy_rate = phyrate_tbl[psta_dframe_info->sta_data_rate].bw40_sgi_phyrate;
							else
								phy_rate = phyrate_tbl[psta_dframe_info->sta_data_rate].bw40_lgi_phyrate;
							break;

						case CHANNEL_WIDTH_80:
							BW = "80 MHz";
							if (psta_dframe_info->sta_sgi)
								phy_rate = phyrate_tbl[psta_dframe_info->sta_data_rate].bw80_sgi_phyrate;
							else
								phy_rate = phyrate_tbl[psta_dframe_info->sta_data_rate].bw80_lgi_phyrate;
							break;

						case CHANNEL_WIDTH_160:
							BW = "160 MHz";
							if (psta_dframe_info->sta_sgi)
								phy_rate = phyrate_tbl[psta_dframe_info->sta_data_rate].bw160_sgi_phyrate;
							else
								phy_rate = phyrate_tbl[psta_dframe_info->sta_data_rate].bw160_lgi_phyrate;
							break;
						...
						/* txpower, qdBm = 4 * dbm */
						current_txpwr = 4 * rtw_get_current_txpwr(padapter, psta);
